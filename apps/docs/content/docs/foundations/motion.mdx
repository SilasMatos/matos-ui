...
- **Tiers** — five springs, named by character rather than by number.
  `fast`/`moderate` are critically damped, `slow` carries a little overshoot,
  `morph` is for shape rather than distance, and `playful` is loud on purpose.
...
| `morph`    | `0.52s`  | `0.06` | Shape, not distance: a `layout` animation changing width, height and radius together. |
| `playful`  | `0.32s`  | `0.4`  | A tone, not a speed. Opt in by hand, never reached by offset.        |
...
hundreds of pixels of width and height at once. At `slow`'s 0.24s that reads as
