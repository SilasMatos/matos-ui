...
<TerminalBlock code="claude mcp add matos-ui -- npx -y @matos-ui/mcp@latest" />
...
<TerminalBlock code="npx -y @matos-ui/mcp@latest" />

Running that bare in a terminal is expected to print a single status line
(`Matos UI MCP server running on stdio.`) and then sit there — it's holding
the connection open on stdin/stdout waiting for a client to speak JSON-RPC,
not frozen. Ctrl+C to exit.
